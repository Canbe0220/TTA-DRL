...
class EnvState:
    ...
    opes_mask_adj: torch.Tensor = None
    opes_mask_job: torch.Tensor = None

    # dynamic
    batch_idxes: torch.Tensor = None
    feat_opes_batch: torch.Tensor = None
    feat_mas_batch: torch.Tensor = None
    proc_pair_batch: torch.Tensor = None
    ...
    def update(self, batch_idxes, feat_opes_batch, feat_mas_batch, proc_pair_batch, proc_times_batch, ope_ma_adj_batch,
        ...
        self.proc_pair_batch = proc_pair_batch
...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, case, env_paras, data_source='case'):
        ...
        self.opes_mask_adj = torch.zeros(self.batch_size, self.num_opes, self.num_opes)
        self.opes_mask_job = torch.zeros(self.batch_size, self.num_opes, self.num_opes)


        for i in range(self.batch_size):
            start_idx = self.num_ope_biases_batch[i]
            end_idx = self.end_ope_biases_batch[i]
            for j, k in zip(start_idx, end_idx):
                for t in range(j, k):
                    self.opes_mask_adj[i, t, t + 1] = 1  
                    self.opes_mask_adj[i, t + 1, t] = 1
                for t in range(j, k + 1):
                    self.opes_mask_adj[i, t, t] = 1

        for i in range(self.batch_size):
            start_idx = self.num_ope_biases_batch[i]
            end_idx = self.end_ope_biases_batch[i]
            for j,k in zip(start_idx, end_idx):
                self.opes_mask_job[i, j:k+1, j:k+1] = 1
        ...
        proc_pair_batch = torch.zeros(size=(self.batch_size, num_jobs, num_mas, self.paras["pair_feat_dim"]))
        ...
        proc_pair_batch[..., 0] = self.ope_ma_adj_batch[batch_indices, start_opes]
        proc_pair_batch[..., 1] = self.proc_times_batch[batch_indices, start_opes].div(self.max_proc)
        proc_pair_batch[..., 2] = proc_pair_batch[..., 1].div(torch.max(proc_pair_batch[..., 1], dim=-1, keepdim=True)[0] + 1e-8)
        proc_pair_batch[..., 3] = proc_pair_batch[..., 1].div(torch.max(proc_pair_batch[..., 1], dim=-2, keepdim=True)[0] + 1e-8)


        feat_mas_batch[:, 0, :] = torch.count_nonzero(self.ope_ma_adj_batch, dim=1).float()
        feat_mas_batch[:, 1, :] = torch.sum(self.proc_times_batch, dim=1).div(feat_mas_batch[:, 0, :] + 1e-8)
        ...
        self.state = EnvState(batch_idxes=self.batch_idxes,
                              ...
                              proc_pair_batch=self.proc_pair_batch,
                              ...
                              opes_mask_adj = self.opes_mask_adj,
                              opes_mask_job = self.opes_mask_job,
        ...
        self.old_proc_pair_batch = copy.deepcopy(self.proc_pair_batch)
    ...
    def step(self, actions):
        ...
        self.reward_batch /= self.max_proc
        ...
        self.proc_pair_batch[self.batch_idxes, jobs, :, 0] = self.ope_ma_adj_batch[self.batch_idxes, next_opes, :].float()
        self.proc_pair_batch[self.batch_idxes, jobs, :, 1] = self.proc_times_batch[self.batch_idxes, next_opes, :].div(self.max_proc)
        active_mask = (~self.mask_job_finish_batch).float().unsqueeze(-1)
        self.proc_pair_batch *= active_mask.unsqueeze(-1)
        self.proc_pair_batch[..., 2] = self.proc_pair_batch[..., 1].div(torch.max(self.proc_pair_batch[..., 1], dim=-1, keepdim=True)[0] + 1e-8)
        self.proc_pair_batch[..., 3] = self.proc_pair_batch[..., 1].div(torch.max(self.proc_pair_batch[..., 1], dim=-2, keepdim=True)[0] + 1e-8)
        
       
        self.feat_mas_batch[self.batch_idxes, 0, :] = torch.count_nonzero(self.ope_ma_adj_batch[self.batch_idxes, ...], dim=-2).float()
        self.feat_mas_batch[self.batch_idxes, 1, :] = torch.sum(self.proc_times_batch[self.batch_idxes, ...], dim=-2) \
                                                      .div(self.feat_mas_batch[self.batch_idxes, 0, :] + 1e-8)
        self.feat_mas_batch[self.batch_idxes, 2, :] = self.mask_ma_procing_batch[self.batch_idxes, :].float()
        self.feat_mas_batch[self.batch_idxes, 3, :] = (self.time[self.batch_idxes].unsqueeze(-1) - self.machines_batch[self.batch_idxes, :, 1]).clamp(min=0)
        ...
        self.state.update(self.batch_idxes, self.feat_opes_batch, self.feat_mas_batch, self.proc_pair_batch, self.proc_times_batch,
    ...
    def reset(self):
        ...
        self.proc_pair_batch = copy.deepcopy(self.old_proc_pair_batch)
